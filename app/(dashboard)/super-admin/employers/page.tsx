...
export const SuspandIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M12 2C17.5 2 22 6.5 22 12C22 17.5 17.5 22 12 22C6.5 22 2 17.5 2 12C2 6.5 6.5 2 12 2ZM12 4C10.1 4 8.4 4.6 7.1 5.7L18.3 16.9C19.3 15.5 20 13.8 20 12C20 7.6 16.4 4 12 4ZM16.9 18.3L5.7 7.1C4.6 8.4 4 10.1 4 12C4 16.4 7.6 20 12 20C13.9 20 15.6 19.4 16.9 18.3Z" fill="#FF0040" />
  </svg>
...
export default function page() {
  ...
  const filteredemployers = employers.filter(learner =>
  ...
  const getStatusColor = (status: string) => {
    switch (status.toLocaleLowerCase()) {
      case 'verified': return 'bg-[#28A74533] text-[#28A745]';
      case 'rejected': return 'bg-[#DBEAFE] text-[#6A0DAD]';
  ...
  return (
        ...
        <table className="w-full text-sm ">
          <thead className="">
            <tr className="text-left border-b border-auth-form-border text-xs xl:text-sm">
              ...
              <th className=" pl-4 min-w-24 ">Company Name</th>
              <th className="  min-w-28 ">Industry</th>
              <th className=" min-w-20 ">Active Jobs</th>
              <th >Status</th>
          ...
          <tbody className="divide-y divide-auth-form-border text-xs xl:text-sm">
            {filteredemployers.map((learner) => (
              <tr key={learner.id} className="hover:bg-gray-50">
                <td className="py-3 text-black font-normal  xl:pl-6 pl-4 ">{learner.id}</td>
                <td className="py-3   text-black font-normal">{learner.companyName}</td>
                <td className="py-3 text-black font-normal">{learner.Industry}</td>
                <td className="py-3 ">
    ...
    
    {
      ...
      {
        isProfile &&
        <EmployerProfileModal onClose={() => setIsProfile(false)} />
      }

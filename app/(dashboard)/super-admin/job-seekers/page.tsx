...
export default function page() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'Normal user': return 'bg-[#28A74533] text-[#28A745]';
      case 'Court referred' : return "bg-[#5A47EA33] text-[#5A47EA]"
        ...
        <table className="w-full text-sm ">
          <thead className="">
            <tr className="text-left border-b border-auth-form-border text-xs xl:text-sm">
              ...
              <th className="  min-w-28 ">Skills</th>
          ...
          <tbody className="divide-y divide-auth-form-border text-xs xl:text-sm">
            {filteredemployers.map((learner) => (
              <tr key={learner.id} className="hover:bg-gray-50">
                ...
                <td className="py-3   text-black font-normal">{learner.name}</td>
